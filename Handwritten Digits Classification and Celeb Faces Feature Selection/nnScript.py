...
def preprocess():
    ...
    new_validation_data = []
    new_test_data = []
    select_features_indexes = []
    count = 0
    for j in range(0,train_data.shape[1]):    
        if(len(np.unique(train_data[:,[j]])) == 1):
            count+=1
        else:
            select_features_indexes.append(j)
            x = train_data[:,j]
            new_train_data.append(x)
            x = test_data[:,j]
            new_test_data.append(x)
            x = validation_data[:,j]
            new_validation_data.append(x)
    train_data = np.transpose(np.asarray(new_train_data))
    test_data = np.transpose(np.asarray(new_test_data))
    validation_data = np.transpose(np.asarray(new_validation_data))
    print('preprocess done')
    
    return train_data, train_label, validation_data, validation_label, test_data, test_label,select_features_indexes
